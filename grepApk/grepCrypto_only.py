...
for app in applist:
    ...
    # grep dex1
    cmd = 'cat %s | grep -e "Ljavax/crypto/Cipher;.getInstance:(Ljava/lang/String;)Ljavax/crypto/Cipher;"' % applog 
    process = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
    ...
    # grep dex2
    cmd = 'cat %s | grep -e "Ljavax/crypto/spec/IvParameterSpec;.<init>:(\\[B"' % applog 
    process = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
    (out, err) = process.communicate()  #TODO error handling
    if out != '':
        resdex_second = 1
    ...
    # grep dex3
    cmd = 'cat %s | grep -e "Lorg/apache/http/conn/ssl/SSLSocketFactory;.setHostnameVerifier:(Lorg/apache/http/conn/ssl/X509HostnameVerifier;)V"' % applog 
    process = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
    (out, err) = process.communicate()  #TODO error handling
    if out != '':
        resdex_three = 1
    ...
    # grep dex4
    cmd = 'cat %s | grep -e "Ljavax/net/ssl/HttpsURLConnection;.setHostnameVerifier:(Ljavax/net/ssl/HostnameVerifier;)V"' % applog 
    process = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
    (out, err) = process.communicate()  #TODO error handling
    if out != '':
        resdex_four = 1
    ...
    # output
    print '[GrepPort] %s\t%d\t%d\t%d\t%d\t%d' % (package, muldex, resdex_first, resdex_second, resdex_three, resdex_four)
    flush()
